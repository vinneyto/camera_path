...
def test_camera_track_defaults_to_forward_without_keyframes() -> None:
    project = Project()

    assert project.camera_track.keyframes == {}
    assert project.camera_track.default_aim.kind == "follow_path"
    assert project.camera_track.default_aim.direction == "forward"


def test_legacy_default_look_at_survives_round_trip() -> None:
    payload = Project().model_dump()
    ...
    assert restored.camera_track.default_aim.kind == "look_at_point"
    assert restored.camera_track.default_aim.scene_point_id == "legacy-target"
    assert reloaded.camera_track.default_aim == restored.camera_track.default_aim
    assert restored.camera_track.keyframes == {}


def test_empty_camera_track_stays_empty_after_round_trip() -> None:
    payload = Project().model_dump()
    payload["camera_track"]["keyframes"] = {}
