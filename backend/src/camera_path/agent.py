...
TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "parameters": _object(
            {"speed": {"type": "number", "exclusiveMinimum": 0}}, ["speed"]
        ),
        "strict": True,
    },
    {
...
class TrajectoryAgent:
    ...
    async def handle(self, project_id: str, message: str) -> ChatResult:
        ...
        for _ in range(12):
            ...
            if not calls:
                ...
                draft.chat_history.extend(
                    [
                        ...
                        ChatHistoryMessage(role="assistant", content=response.output_text),
                    ]
                )
            ...
            for call in calls:
                ...
                output = self._execute(draft, call.name, arguments)
                inputs.append(
                    {
    ...
    @classmethod
    def _execute(cls, draft: Any, name: str, arguments: dict[str, Any]) -> Any:
