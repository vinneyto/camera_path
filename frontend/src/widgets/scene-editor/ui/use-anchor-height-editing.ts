...
export function useAnchorHeightEditing({ anchors, onCommit }: UseAnchorHeightEditingOptions) {
  ...
  useEffect(() => {
    if (dragRef.current !== null && activeTool !== "anchor-height") cancelDrag();
  ...
  const handlePointerUp = useCallback((anchor: Anchor, event: ThreeEvent<PointerEvent>) => {
    const drag = dragRef.current;
    if (drag?.anchorId !== anchor.id || drag.pointerId !== event.pointerId) return;
    event.stopPropagation();
    drag.target?.releasePointerCapture?.(drag.pointerId);
  ...
  const handlePointerCancel = useCallback((anchor: Anchor, event: ThreeEvent<PointerEvent>) => {
    if (dragRef.current?.anchorId !== anchor.id) return;
    event.stopPropagation();
    cancelDrag();
  }, [cancelDrag]);

  const activeAnchorId = preview?.anchorId ?? (activeTool === null ? hoveredAnchorId : null);
