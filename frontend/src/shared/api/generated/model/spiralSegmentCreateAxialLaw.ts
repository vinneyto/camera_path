/**
 * Generated by orval 🍺
 * Do not edit manually.
 * Camera Path API
 * Create camera-path projects, author scene and trajectory data, edit playback timelines, and run the trajectory agent. Canonical operations use project revision ETags for optimistic concurrency.
 * OpenAPI spec version: 1.0.0
 */

export type SpiralSegmentCreateAxialLaw = typeof SpiralSegmentCreateAxialLaw[keyof typeof SpiralSegmentCreateAxialLaw];


export const SpiralSegmentCreateAxialLaw = {
  linear: 'linear',
  smoothstep: 'smoothstep',
} as const;
